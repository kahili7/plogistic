...
class Tableall extends Crmcontroller
{

    public function __construct()
    {
	parent::__construct();
	$this->lang->load('tableall');
	$this->load->model('tableall_model');
    }
    ...
    {
	$data = array();
	$data['title'] = lang('TABLEALL_TITLE');
	$data['orid'] = $this->get_orid();
	$data['sort'] = $this->get_session('sort');
	$data['find'] = $this->find();
	$data['fields']['id_warehouse'] = array('label' => lang('ID_WAREHOUSE'), 'colwidth' => '10%', 'sort' => TRUE);
	$data['fields']['id_group'] = array('label' => lang('ID_GROUP'), 'colwidth' => '10%', 'sort' => TRUE);
	$data['fields']['id_articule'] = array('label' => lang('ID_ARTICULE'), 'colwidth' => '10%', 'sort' => TRUE);
	$data['fields']['name'] = array('label' => lang('NAME'), 'colwidth' => '70%', 'sort' => TRUE);
	$data['tools'] = $this->get_tools();
	$data['ds'] = $this->tableall_model->get_ds();
	$data['paging'] = $this->get_paging($this->tableall_model->get_calc_rows());
	...
	if ($this->form_validation->run() === TRUE)
	{
	    if ($this->contacts_model->update_record())
		$data['success'] = TRUE;
	    else
		$data['success'] = FALSE;
	...
	if ($this->form_validation->run() == TRUE)
	{
		...
		$search_rule['like']['_table_all.name'] = $this->input->post('name');
	...
	return $this->load->view('tableall/find', $data, TRUE);
    }
	...
	if ($this->form_validation->run() === TRUE)
	{
	    if ($this->contacts_model->move_record())
		$data['success'] = TRUE;
	    else
		$data['success'] = FALSE;
	...
	if ($this->form_validation->run() == TRUE)
	{
	    ...
	    if ($this->input->post('name'))
		$search_rule['like']['_contacts.name'] = $this->input->post('name');
	    if ($this->input->post('contface'))
		$search_rule['like']['_contacts.contface'] = $this->input->post('contface');
	    if ($this->input->post('phone'))
		$search_rule['like']['_contacts.phone'] = $this->input->post('phone');
	    if ($this->input->post('email'))
		$search_rule['like']['_contacts.email'] = $this->input->post('email');
