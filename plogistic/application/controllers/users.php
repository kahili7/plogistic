...
class Users extends Crmcontroller
{
	...
	if ($this->form_validation->run() === TRUE)
	{
	    if ($this->users_model->update_record())
		$data['success'] = TRUE;
	    else
		$data['success'] = FALSE;
	    $data['ds'] = $this->users_model->get_edit($rid);
	...
	{
	    $search_rule = array();
	    if ($this->input->post('_employeers_rid'))
		$search_rule['where']['_employeers.rid'] = $this->input->post('_employeers_rid');
	    if ($this->input->post('user_login'))
		$search_rule['like']['_users.user_login'] = $this->input->post('user_login');
	...
	if ($this->form_validation->run() === TRUE)
	{
	    if ($this->users_model->move_record())
		$data['success'] = TRUE;
	    else
		$data['success'] = FALSE;
	    $data['ds'] = $this->users_model->get_edit($rid);
