...
class XML_RPC_Client extends KI_XMLRPC
{
	var $path			= '';
	var $server			= '';
	var $port			= 80;
...
class XML_RPC_Response
{
	...
	function decode($array=FALSE)
	{
		...
		if ($this->xss_clean && ! isset($KI->security))
		{
			$KI->load->library('security');
		}
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($this->xss_clean) ? $KI->security->xss_clean($array[$key]) : $array[$key];
				}
			}
		...
		else
		{
			...
			{
				$result = ($this->xss_clean) ? $KI->security->xss_clean($result) : $result;
			}
		}
...
class XML_RPC_Message extends KI_XMLRPC
{
	var $payload;
	var $method_name;
	var $params			= array();
	...
	function output_parameters($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($key == 'bits' OR $this->xss_clean == FALSE) ? $array[$key] : $KI->security->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			for ($i = 0; $i < count($this->params); $i++)
			{
				...
				{
					$parameters[] = ($this->xss_clean) ? $KI->security->xss_clean($a_param) : $a_param;
				}
			}
		}
...
class XML_RPC_Values extends KI_XMLRPC
{
	var $me		= array();
	var $mytype	= 0;
