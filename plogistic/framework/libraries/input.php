...
class KI_INPUT
{
	...
	function get_post($index='', $xss_clean=FALSE)
	{
		...
		else return $this->post($index, $xss_clean);
	}
	...
	function ip_address()
	{
		...
		else if($this->server('REMOTE_ADDR') AND $this->server('HTTP_CLIENT_IP'))
		{
	...
	function valid_ip($ip)
	{
		...
		if(count($ip_segments) != 4) return FALSE;
		if($ip_segments[0][0] == '0') return FALSE;
	...
	function user_agent()
	{
		if($this->user_agent !== FALSE) return $this->user_agent;
	...
	function xss_clean($str, $is_image=FALSE)
	{
		...
		if($is_image === TRUE)
		{
			if($str == $converted_string) return TRUE;
			else return FALSE;
		}
