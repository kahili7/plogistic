...
class KI_JAVASCRIPT
{
	...
	$this->KI->load->library('javascript/' . $js_library_driver, array('autoload' => $autoload));
	$this->js = & $this->KI->$js_library_driver;
	...
	{
	    if ($this->KI->config->item('javascript_location') != '')
	    {
		$this->_javascript_location = $this->KI->config->item('javascript_location');
	    }
	}
	...
	{
	    $str = $this->_open_script($this->KI->config->slash_item('base_url') . $this->_javascript_location . $external_file);
	}
    ...
    {
	$str = '<script type="text/javascript" charset="' . strtolower($this->KI->config->item('charset')) . '"';
	$str .= ( $src == '') ? '>' : ' src="' . $src . '">';
	return $str;
    }
