...
class KI_CALENDAR
{
	...
	function KI_CALENDAR($config=array())
	{
		$this->KI =& get_instance();

		if(!in_array('calendar_lang'.EXT, $this->KI->lang->is_loaded, TRUE))
		{
			$this->KI->lang->load('calendar');
		}
	...
	function get_month_name($month)
	{
		...
		if($this->KI->lang->line($month) === FALSE)
		{
			return ucfirst(str_replace('cal_', '', $month));
		}

		return $this->KI->lang->line($month);
	}
	...
	function get_day_names($day_type='')
	{
		...
		{
			$days[] = ($this->KI->lang->line('cal_'.$val) === FALSE) ? ucfirst($val) : $this->KI->lang->line('cal_'.$val);
		}
	...
	{
		return  array(
		'table_open' 				=> '<table border="0" cellpadding="4" cellspaKIng="0">',
