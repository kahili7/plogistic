...
class KI_FORM_VALIDATION
{
    ...
    function KI_FORM_VALIDATION($rules=array())
    {
	$this->KI = & get_instance();
	$this->_config_rules = $rules;
	$this->KI->load->helper('form');
	...
	if (count($this->_field_data) == 0)
	{
	    ...
	    $uri = ($group == '') ? trim($this->KI->uri->ruri_string(), '/') : $group;
	...
	$this->KI->lang->load('form_validation');
	...
	if (is_null($postdata) AND $callback == FALSE)
	{
		...
		{
		    if (FALSE === ($line = $this->KI->lang->line($type)))
		    {
	...
	foreach ($rules As $rule)
	{
		...
		$result = $this->KI->$rule($postdata, $param);
	...
	if (substr($fieldname, 0, 5) == 'lang:')
	{
	    ...
	    if (FALSE === ($fieldname = $this->KI->lang->line($line)))
	    {
		return $line;
	    }
	}
    ...
    {
	return $this->KI->input->strip_image_tags($str);
    }

    function xss_clean($str)
    {
	return $this->KI->input->xss_clean($str);
    }
